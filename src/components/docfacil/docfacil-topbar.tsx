...
export function DocfacilTopbar({ backHref, onBack, title, subtitle, actions }: DocfacilTopbarProps) {
  const backButton = (
    <span className="flex items-center gap-1.5 text-sm font-medium text-zinc-500 hover:text-zinc-900 transition-colors">
      <ArrowLeft className="h-4 w-4" /> Voltar
    </span>
  );

  return (
    <header className="flex items-center gap-4 border-b border-zinc-200 bg-zinc-50 px-4 sm:px-6 h-14 no-print">
      {backHref ? (
