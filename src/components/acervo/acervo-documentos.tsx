...
/** Ficha de filtro da faixa superior (Todas / pastas / Lixeira). Substituiu a
 *  coluna lateral, que gastava um quarto da largura pra mostrar um contador,
 *  uma lista de pastas quase sempre vazia e um link de lixeira. */
function FichaFiltro({
  ativa,
  tom = 'normal',
  onClick,
  children,
...
export type EscopoAcervo = 'autuacoes' | 'relatorios';

type OrdenacaoAcervo = 'recentes' | 'antigos' | 'nome' | 'prazo';

export type AcervoDocumentosProps = {
  titulo: string;
  subtitulo: string;
  /** Que tipo de documento esta tela lista. */
  escopo: EscopoAcervo;
  /**
   ...
   *  única saída seria o "Início" do cabeçalho global, que vai pra dashboard. */
  voltarPara: string;
};

/**
 * ACERVO — a lista de documentos com todo o aparato de arquivo: busca, pastas
 * de trabalho, lixeira (com restaurar e excluir definitivo), seleção em lote,
 * download em ZIP, favoritos, filtro por fiscal, ajuste de prazo e relatório
 * municipal anual.
 *
 * Era a tela única "Documentos", que misturava autuação e relatório de
 * vistoria, rascunho e finalizado, num lugar só — cheia demais pra quem não é
 * da área. Agora é um componente parametrizado, e cada recorte tem sua própria
 * tela, alcançada por um cartão no menu de Autuações ou de Roteiros
 * (ver MenuHub): em andamento, finalizadas/relatórios.
 *
 * O que é específico de autuação (prazo legal, relatório municipal, aviso de
 * limite de armazenamento) só aparece quando `escopo` é 'autuacoes'.
 */
export function AcervoDocumentos({ titulo, subtitulo, escopo, situacao, novo, voltarPara }: AcervoDocumentosProps) {
  ...
  // desta tela alimenta a lista — ver `autuacoes`/`relatorios` logo abaixo.
  const {
    inspecoes,
  ...
  const documentos = useMemo<DocumentoUnificado[]>(() => {
    const doIntimacoes: DocumentoUnificado[] = autuacoes.map(i => ({
  ...
  }, [autuacoes, relatorios, roteirosPorId]);
  ...
  /** Padrão: mais recentes primeiro — é o que se procura em quase todo acesso
   *  ("o que eu mexi por último"). As demais ordens ficam à escolha. */
  const [ordenacao, setOrdenacao] = useState<OrdenacaoAcervo>('recentes');
  ...
  const filteredDocumentos = useMemo(() => {
    return documentos
      ...
      .sort((a, b) => {
        ...
        // cada item), independentemente da ordenação escolhida.
        const porFavorito = Number(b.favorito) - Number(a.favorito);
        if (porFavorito !== 0) return porFavorito;

        // A ordenação usa a MESMA data que aparece na linha. Ordenar por um
        // campo invisível (updatedAt, por exemplo) faria a lista parecer
        // embaralhada: o usuário veria "09 set" acima de "12 set" sem
        // explicação na tela.
        const quandoA = a.data ? new Date(a.data).getTime() : 0;
        const quandoB = b.data ? new Date(b.data).getTime() : 0;

        switch (ordenacao) {
          case 'antigos':
            return quandoA - quandoB;
          case 'nome':
            return a.assunto.localeCompare(b.assunto, 'pt-BR');
          case 'prazo': {
            // Vencendo primeiro. Documento sem prazo (rascunho, relatório) vai
            // pro fim, senão empurraria pra cima justamente o que não tem
            // prazo nenhum pra acompanhar.
            const prazoA = a.intimacao ? calculateDeadline(a.intimacao)?.remaining : undefined;
            const prazoB = b.intimacao ? calculateDeadline(b.intimacao)?.remaining : undefined;
            if (prazoA === undefined && prazoB === undefined) return quandoB - quandoA;
            if (prazoA === undefined) return 1;
            if (prazoB === undefined) return -1;
  ...
  }, [documentos, searchQuery, activeFolderId, filterByFiscal, ordenacao]);
  ...
  return (
    <div className="min-h-screen bg-[#F5F2EA]">
      <DocfacilTopbar
        backHref={voltarPara}
        title={titulo}
        subtitle={isRoot ? (selectedMunicipioForRoot || "Selecione um município") : subtitulo}
        actions={isRoot ? (
      ...
      <div className="max-w-5xl mx-auto w-full p-4 sm:p-8 pb-40 space-y-4">
        ...
        <>
          {/* Busca + ação principal. O título e a explicação da tela já estão na
              barra do topo (DocfacilTopbar) — repetir aqui era a maior fonte de
              ruído da versão anterior. */}
          ...
          {/* Filtros como faixa de fichas, no lugar da coluna lateral de antes:
              ocupava um quarto da tela pra mostrar um contador, uma lista de
              pastas quase sempre vazia e um link de lixeira. */}
          <div className="flex flex-wrap items-center gap-1.5">
            <FichaFiltro
              ativa={activeFolderId === 'all' && !filterByFiscal}
              onClick={() => { setActiveFolderId('all'); setFilterByFiscal(null); setSelectedIds([]); }}
            >
              Todas <span className="tabular-nums opacity-60">{documentos.filter(d => !d.deleted).length}</span>
            </FichaFiltro>

            {folders.map(folder => (
              <FichaFiltro
                key={folder.id}
                ativa={activeFolderId === folder.id}
            ...
            <div className="w-full sm:w-auto sm:ml-auto flex flex-col sm:flex-row items-stretch sm:items-center gap-1.5">
              <Select value={ordenacao} onValueChange={(v) => setOrdenacao(v as OrdenacaoAcervo)}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="antigos" className="text-xs">Mais antigos</SelectItem>
                  <SelectItem value="nome" className="text-xs">Estabelecimento (A-Z)</SelectItem>
                  {/* Prazo de defesa só corre em autuação finalizada. */}
                  {escopo === 'autuacoes' && situacao === 'finalizado' && (
                    <SelectItem value="prazo" className="text-xs">Prazo mais próximo</SelectItem>
                  )}
                </SelectContent>
              </Select>

              {/* Filtro por fiscal em menu, não em lista: com 20 fiscais numa
                  coluna lateral, a tela virava um diretório de pessoas. */}
              {isGestor && equipeFiscais.length > 0 && (
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="todos" className="text-xs">Todos os fiscais</SelectItem>
                    {equipeFiscais.map(fiscal => (
                      <SelectItem key={fiscal.id} value={fiscal.id} className="text-xs">{fiscal.nome}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              )}
