...
export function SelecionarAutoridadeParaFormulario({ onSelect }: { onSelect: (autoridade: Autoridade) => void }) {
    ...
    // Vários fiscais marcados antes de confirmar de uma vez — antes, cada
    // toque já chamava onSelect e fechava o diálogo, obrigando a reabrir pra
            ...
            >
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <DialogDescription className="text-zinc-400 text-[10px] font-bold uppercase tracking-widest mt-1">
                                Marque um ou mais e confirme no final
                            </DialogDescription>
                        </div>
                    </div>
                </DialogHeader>
                ...
                <div className="p-4 sm:p-6 bg-white flex flex-col gap-4">
                            ...
                            {loading ? (
                                ...
                                <div className="space-y-2">
                                    {filteredAutoridades.map(autoridade => {
                                        const selected = selectedIds.has(autoridade.id);
                                        return (
                                        <div
                                            key={autoridade.id}
                                            onClick={() => toggleSelected(autoridade.id)}
                                            className={cn(
                                        ...
                                        >
                                            <div className="flex items-center gap-4 flex-grow">
                                                <div className={cn(
                                                    ...
                                                    selected ? "bg-primary text-white" : "bg-zinc-200 text-zinc-500"
                                                )}>
                    ...
                    <Button
                        ...
                        className="w-full h-12 rounded-2xl bg-primary hover:bg-primary/90 text-white font-black uppercase text-xs tracking-widest gap-2 shadow-lg shadow-primary/20 disabled:opacity-40"
                    >
                        <Check className="h-4 w-4" />
                        {selectedIds.size > 0 ? `Adicionar ${selectedIds.size} selecionado${selectedIds.size > 1 ? 's' : ''}` : 'Selecione ao menos um fiscal'}
                    </Button>
                </div>
            </DialogContent>
        </Dialog>
