...
export default function GestaoEquipePage() {
  ...
  useEffect(() => {
    ...
    const loadMasterList = () => {
      try {
        const locals = JSON.parse(localStorage.getItem(MASTER_DB_KEY) || '[]');
        // Se for gestor, filtra apenas o seu município
        if (profile.role === 'admin') {
            const mid = normalizeId(profile.municipioId);
            return locals.filter((u: any) => normalizeId(u.municipioId) === mid);
        }
        return locals;
      } catch (e) {
        return [];
  ...
  const handleUpdateFiscalCode = async (userId: string, email: string, code: string) => {
    ...
    const newData = { fiscalCode: cleanCode };
    
    const localList = JSON.parse(localStorage.getItem(MASTER_DB_KEY) || '[]');
    const updatedLocal = localList.map((u: any) => 
        u.email.toLowerCase() === email.toLowerCase() ? { ...u, ...newData } : u
    );
    localStorage.setItem(MASTER_DB_KEY, JSON.stringify(updatedLocal));
    setUsers(prev => prev.map(u => u.email.toLowerCase() === email.toLowerCase() ? { ...u, ...newData } : u));

    if (db && !userId.toString().startsWith('local-')) {
  ...
  const toggleAuth = async (userId: string, currentStatus: boolean, userEmail: string) => {
    ...
    const newData: any = { 
        ...
        status: isApproving ? 'approved' : 'pending',
    ...
    const localList = JSON.parse(localStorage.getItem(MASTER_DB_KEY) || '[]');
    const updatedLocal = localList.map((u: any) => 
        u.email.toLowerCase() === userEmail.toLowerCase() ? { ...u, ...newData } : u
    );
    localStorage.setItem(MASTER_DB_KEY, JSON.stringify(updatedLocal));
    setUsers(prev => prev.map(u => u.email.toLowerCase() === userEmail.toLowerCase() ? { ...u, ...newData } : u));
  ...
  const displayCity = profile?.municipioNome || profile?.municipioId?.toUpperCase() || "SISTEMA";
  const filteredUsers = users.filter(u => 
    (u.displayName || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
    (u.email || "").toLowerCase().includes(searchTerm.toLowerCase())
  );
  
  const pendingUsers = filteredUsers.filter(u => !u.isAuthorized && u.role !== 'root');
  ...
  return (
    <div className="max-w-6xl mx-auto w-full p-4 sm:p-8 space-y-10 font-sans pb-32">
      ...
      <div className="bg-white border-2 border-slate-200 rounded-[2.5rem] p-4 flex items-center shadow-xl">
            ...
            <section className="space-y-4">
                ...
                <div className="grid gap-4">
                    ...
                    <div key={u.uid || u.id} className="bg-white border-2 border-amber-100 p-6 rounded-[2.5rem] flex flex-col sm:flex-row items-center justify-between gap-6 hover:shadow-xl transition-all animate-in fade-in zoom-in">
                        ...
                        <Button onClick={() => toggleAuth(u.id || u.uid, false, u.email)} className="w-full sm:w-auto h-12 px-8 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-black uppercase text-[10px] shadow-lg transition-all active:scale-95">
        ...
        <section className="space-y-8">
            <div className="flex items-center justify-between px-2">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[8px] font-black text-zinc-300 uppercase tracking-widest">Sincronizado com Cofre V6</span>
                </div>
            </div>
