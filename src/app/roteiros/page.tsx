...
export default function RoteirosPage() {
  ...
  const roteirosMunicipais = orderedRoteiros.filter(r => 'municipioId' in r);
  const roteirosRoi = orderedRoteiros.filter(r => 'tipo' in r && r.tipo === 'roi');
  const roteirosGerais = orderedRoteiros.filter(r => !('municipioId' in r) && !('tipo' in r));
          ...
          <div className="space-y-2">
            ...
            <RoteiroSection
              ...
              onDragEnd={(event) => {
                ...
                if (!over) return;
                reordenarGrupo(roteirosMunicipais.map((r) => r.id), String(active.id), String(over.id));
              }}
            />
          </div>
        )}
          ...
          <div className="space-y-2">
            ...
            <RoteiroSection
              ...
              onDragEnd={(event) => {
                const { active, over } = event;
                if (!over) return;
                reordenarGrupo(roteirosGerais.map((r) => r.id), String(active.id), String(over.id));
              }}
            />
          </div>
        )}
          ...
          <div className="space-y-3">
            ...
            <RoteiroSection
              ...
              onDragEnd={(event) => {
                const { active, over } = event;
                if (!over) return;
                reordenarGrupo(roteirosRoi.map((r) => r.id), String(active.id), String(over.id));
              }}
            />
          </div>
        )}
